...
int SSLInspectInit(struct thread *thread);
int FstackHandler(void *pthread);
...
int FstackHandler(void *pthread)
{
    pcurthread = (struct thread *)pthread;
    ...
    for (i = 0; i < nevents; ++i) {
        ...
        if (event.flags & EV_EOF) {
            ...
            ssize_t writelen = ff_write(clientfd, "test", sizeof("test") - 1);
            if (writelen < 0) {
...
#ifdef SSL_INSPECT
    ff_init_for_sc(1, &argv[1], &flow_offset, sc_hook);
    SSLInspectInit(pcurthread);
#endif    
